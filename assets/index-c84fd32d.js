...
`;const u=document.querySelector("#navBar");u==null||u.addEventListener("click",function(){console.log("click")});const p=document.getElementById("next-btn"),h=document.getElementById("prev-btn"),a=document.querySelectorAll(".img-container"),l=document.querySelector(".image-view"),c=document.querySelector(".image-box");let r=0;function w(){let e="";for(let t=0;t<s.length;t++){const o=s[t];e+=`
      <article>
  ...
  `}return e}function x(e){l&&c&&(l.style.display="block",c.style.display="block",r=e,d(r),B(),l.addEventListener("click",y),window.addEventListener("keyup",v))}function B(){l&&c&&a.forEach(e=>{l.style.display==="block"?e.setAttribute("tabindex","-1"):e.removeAttribute("tabindex")})}function y(){l&&c&&(l.style.display="none",c.style.display="none",S(),l.removeEventListener("click",y),window.removeEventListener("keyup",v))}function S(){a.forEach(e=>{e.removeAttribute("tabindex")})}function v(e){e.key==="Escape"&&y()}a.forEach((e,t)=>{e.addEventListener("click",()=>{l&&c&&x(t)})});function T(e){let t="";if(a){t=s[e].imgText;const o=document.querySelector(".image-text-container");o&&(o.innerHTML=`<p>${t}</p>`)}}function d(e){if(c&&a){const t=a[e].querySelector("img"),o=t==null?void 0:t.getAttribute("src");o&&(c.style.background=`url(${o}) center/cover no-repeat`,T(e))}}h&&h.addEventListener("click",function(){r--,r<0&&(r=a.length-1),d(r)});p&&p.addEventListener("click",function(){r++,r===a.length&&(r=0),d(r)});
