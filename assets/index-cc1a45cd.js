...
`;const y=document.getElementById("next-btn"),g=document.getElementById("prev-btn"),a=document.querySelectorAll(".img-container"),l=document.querySelector(".image-view"),c=document.querySelector(".image-box");let r=0;function w(){let e="";for(let t=0;t<s.length;t++){const n=s[t];e+=`
      <article>
  ...
  `}return e}function k(e){l&&c&&(l.style.display="block",c.style.display="block",r=e,d(r),S(),l.addEventListener("click",u),window.addEventListener("keyup",h))}function S(){l&&c&&a.forEach(e=>{l.style.display==="block"?e.setAttribute("tabindex","-1"):e.removeAttribute("tabindex")})}function u(){l&&c&&(l.style.display="none",c.style.display="none",B(),l.removeEventListener("click",u),window.removeEventListener("keyup",h))}function B(){a.forEach(e=>{e.removeAttribute("tabindex")})}function h(e){e.key==="Escape"&&u()}a.forEach((e,t)=>{e.addEventListener("click",()=>{l&&c&&k(t)})});function J(e){let t="";if(a){t=s[e].imgText;const n=document.querySelector(".image-text-container");n&&(n.innerHTML=`<p>${t}</p>`)}}function d(e){if(c&&a){const t=a[e].querySelector("img"),n=t==null?void 0:t.getAttribute("src");n&&(c.style.background=`url(${n}) center/cover no-repeat`,J(e))}}g&&g.addEventListener("click",function(){r--,r<0&&(r=a.length-1),d(r)});y&&y.addEventListener("click",function(){r++,r===a.length&&(r=0),d(r)});
